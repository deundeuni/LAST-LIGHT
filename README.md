> **Multilingual Notice:** This document is published in dual languages (Korean/English). v5.2 2026-09-14 (Korean version: [README.ko.md](README.ko.md))  
...
## Subtitle: Defensive Publication White Paper on 0-Point Calibration, Multisensory Haptics, and Blackbox Infrastructure via ISO 7010 Signage, Hydrant Boxes, Smart Raceways, IMO SOLAS Shipboard Infrastructure, Deep Underground Tunnel Chambers, Orbital Modules, and AR Glasses/Smart Ring/Robotics Infrastructure (Ver. 5.2)
...
* **Original Intellectual Property (IP) Holder:** deundeuni (Project: soma-moa)
...
* **Keywords:** LAST-LIGHT, Evacuation Guidance, Position Calibration, Infrastructure Anchoring, Structure First Principle, ISO 7010, IMO SOLAS, LLL(Low Location Lighting), Fire-Rated Bulkhead, Muster Station, Deep Underground Tunnel, Co-Survival Bridge, Haptic Compass, BLE Auracast, Tri-State Isolation, T-Reg Suppressor, Distributed Local Blackbox, Fire-Rated Hydrant Chamber, Small Cell DAS Integration, Space Station Evacuation, Microgravity Evacuation, Orbital Module Anchor, IVA/EVA Spacesuit Haptics, AR Glasses HUD, Smart Ring, WebAR Quick Release, Visual SLAM Rapid Drift Reset, Prior Art, smart-system-multi-survival-architecture, chiplet-apu-multi-system-survival-architecture, ARCHITECTURE_STRATEGY, CWP, MAX-LIFE-ICE-BELT
...
In fires and disasters, the site faces extreme sensory deprivation: severe darkness (visual isolation) due to heavy smoke and power outages, coupled with auditory paralysis (auditory isolation) caused by emergency sirens exceeding 100 dB and crowd panic noise. Furthermore, during fires inside deep underground malls/tunnels, zero-gravity/microgravity orbital modules (space stations), and sealed passenger/cruise ship cabin corridors, the unconstrained spread of smoke, ship listing, and total darkness render overhead visual emergency lights useless, creating an extreme loss of sensory orientation. Traditional voice announcements or upper visual signs may be significantly degraded in functionality under these conditions.

Addressing this, and aiming to mitigate the fundamental limits of position drift or signal shadow inherent in existing wireless positioning or simple inertial navigation (IMU) devices within indoor, deep underground, maritime shipboard, and space environments, this system conceived an "Infrastructure Anchoring" evacuation guidance mechanism. By establishing permanently installed statutory infrastructure (24/7 illuminated ISO 7010 emergency exit signs, IMO SOLAS Low Location Lighting LLL, indoor hydrant/fire alarm terminal boxes, ceiling/wall raceways, ship muster station signs) in buildings, ship fire-rated bulkheads, and orbital modules as "Visual, Optical, Acoustic, and Wireless RF Absolute Reference Points (ANCHOR)," it real-time calibrates spatial position to 0.

Through this framework, the system guides general evacuees via intuitive tactile direction delivered through bones and skin via personal or spacesuit-embedded "Co-Survival Bridges," AR glasses 3D HUDs, and smart rings without relying on sound, sight, or language barriers. Simultaneously, by embedding a "Distributed Heat-Resistant Local Blackbox" in hydrant boxes, ship fire-rated bulkheads, and module fire-rated compartments to store recent evacuation trajectories in a ring buffer, a self-evolving AI framework for post-disaster root-cause analysis and data-driven evacuation route optimization was established.
...
* **Structure First Principle & Master Survival Philosophy Linkage (Structure First & Topological Anchoring):** This system adopts the topological alignment of existing physical structures (pictograms, hydrant boxes, raceways, SOLAS fire-rated bulkheads, etc.) as its primary design principle, rather than prioritizing sensor placement. All sensors function as subordinate auxiliary tools to this structure. This 'Structure First' principle shares the same design philosophy as the 'Structure over Capacity' principle in Section 0.2 of the master upper survival architecture `smart-system-multi-survival-architecture` and the fault-tolerant design of `chiplet-apu-multi-system-survival-architecture`, materializing it as a spatial/topological anchoring method for disaster evacuation environments.
* **Infrastructure Anchoring Override:** To structurally mitigate cumulative position drift and communication shadow limitations of devices relying solely on inertial navigation or external wireless signals in indoor, deep underground, maritime shipboard, and space environments, this system declares the calibration mechanism utilizing statutory fixed infrastructure signs and modules as absolute 0-point anchors (L0 Anchor) as its core differentiator and primary scope.
* **Non-Obvious Fire-Rated Infrastructure Selection:** The L0 anchor media of this system—such as indoor hydrant boxes, fire alarm terminal boxes, and ship fire-rated bulkhead (Bulkhead) chambers—are non-combustible and fire-rated facilities required by relevant fire safety regulations (e.g., National Fire Performance Codes / NFPC, IMO SOLAS regulations) to maintain physical and structural integrity during fires and flooding. Grounded in the legal and physical assurance of survival during disasters (unlike standard commercial beacons or decorative interior markers), this system purposefully integrates fire-rated hydrant boxes and compartments as absolute 0-point anchors and local blackbox receptacles, establishing non-obvious distinctiveness over conventional beacon placement methods.
* **Subordinate Telecom Infrastructure & Offline-First Hierarchy:** When 5G/6G small cells or Distributed Antenna Systems (DAS) are deployed in underground, tunnel, or shipboard lower deck environments, this system may additionally utilize them as auxiliary position fusion signals and management backhaul paths. However, assuming communication blackouts during disasters, the absolute reference points (ANCHOR) of this system maintain a strict primary hierarchy anchored to fixed infrastructure (ISO 7010 signs, IMO SOLAS LLL, hydrant boxes, raceways) independent of wireless network availability, establishing a master-subordinate relationship where telecom infrastructure functions strictly as a supplementary option.
* **Form-Factor & Stimulus Agnostic Rule:** The wearable receiver of this guidance system is not limited to earphones, headphones, or bone conduction devices. It comprehensively encompasses smartwatches, wristbands, smart rings, AR/MR smart glasses and HUDs, patches, haptic vests, smart shoes/insoles, clothing-embedded modules, spacesuit (IVA/EVA) inner linings, ship lifejacket-embedded transducers, autonomous relief robot (AMR)/drone vision modules, and all other human/machine contact forms. Furthermore, the tactile stimulus mechanism delivering direction is not restricted to Eccentric Rotating Mass (ERM) or Linear Resonant Actuators (LRA); it encompasses local pressure differentials, electrical stimulation, micro-temperature shifts, piezoelectric haptics, and all meaningful drive methods allowing intuitive perception of direction, preempting them under this prior art baseline reference.
* **Master Architecture & Interconnected Hardware System Linkage:** The 0-point anchor calibration, 100ms self-healing mesh, and Tri-State isolation control of LAST-LIGHT constitute a domain application implementation of the bypass and local isolation logic of the master upper survival architecture `smart-system-multi-survival-architecture`, master fault-tolerant survival architecture `chiplet-apu-multi-system-survival-architecture`, and its embedded strategy `ARCHITECTURE_STRATEGY.md` adapted to disaster evacuation environments. The raceway/hydrant box/ship bulkhead attachment represents an infrastructure implementation of CWP's four hardware repository mechanisms, while the 100ms isolation lifespan extension algorithm shares the same survival control philosophy as `MAX-LIFE-ICE-BELT`.

### 0.3 Zero-Downtime & Non-Contact Principle
This structural design strictly prohibits physical or electrical modification of existing statutory fire, building, maritime, or space module infrastructure. It prioritizes organic zero-downtime fail-over capacity, aiming to ensure the entire system does not halt even if individual modules are destroyed by impact or fire. Each module maintains an independent multi-mesh state to mitigate single points of failure (SPOF), allowing adjacent modules to immediately assume computation and blackbox self-logging.

### 0.4 Non-Exclusive Interoperability & Open Public Standard
This structural design is not proprietary to specific optical cameras, vision sensors, microphone sensors, LiDAR, mobile devices, Co-Survival Bridges, AR glasses, smart rings, spacesuit HMD/haptic transducers, or robot vision frameworks. It operates as an Open Public Standard capable of referencing public-domain 24/7 illuminated visual standard signs such as ISO 7010 series and IMO SOLAS low location lighting, hydrant location indicators (internal/external), ship muster station lights, acoustic beacons, BLE Auracast transmitters, and raceway optical/acoustic/RF anchors as auxiliary references.
...
This structural design comprehensively applies to smartphone evacuation applications, Co-Survival Bridges, AR/MR smart glasses Visual HUD navigation, smart rings, wristband/ring/apparel/vest/insole wearable haptic receivers, spacesuit (IVA/EVA) embedded haptic evacuation pads, maritime passenger/cruise ship enclosed cabin compartments and IMO SOLAS Low Location Lighting (LLL) linked receivers, deep underground tunnel/evacuation connection passage anchors, distributed blackbox data forensics modules, autonomous relief robots (AMR) and orbital exploration robot emergency re-calibration modules, smart emergency lighting/acoustic anchors in buildings, ships, and space station modules, hydrant internal/external position indicators and acoustic anchors, underground parking/spacecraft/ship raceway anchors, underground telecom small cell/DAS auxiliary integration modules, disaster rescue AR glasses, firefighter/astronaut/maritime rescuer NVG/thermal and bone-conduction equipment, and powerless passive QR/ArUco/NFC tag-based WebAR and OS deep-link instant onboarding modules requiring visual, optical, auditory, vibration, or tactile perception-based auxiliary positioning.

### 0.7 Disclosure Purpose & Limitation Notice
This document is published as a Defensive Publication / Prior Art disclosure. Functions, configurations, and performance descriptions herein are exemplary and do not limit or guarantee specific implementations. This system does not replace, alter, or extend the functions, performance, or statutory duties of existing legal fire safety equipment or IMO international ship safety equipment, operating strictly for auxiliary and reference purposes.
...
* **v4.8 (2026-09-09):** Explicitly declared 'Structure First' principle and cross-references. Standardized Appendix C AI disclosures into a generalized AI tools framework (`AI models`) without specific corporate brand names.
* **v4.9 (2026-09-14):** Encompassed maritime shipboard and deep underground tunnel anchor territory (IMO SOLAS LLL, fire-rated Bulkheads, Muster Stations).
* **v5.0 (2026-09-14):** Ecosystem citation and upper architecture synchronization patch applied.
* **v5.1 (2026-09-14):** Real Zenodo DOI and `deundeuni` account correction patch applied.
* **v5.2 (2026-09-14):** **[Appendix A Primary Repository Account deundeuni Consistency Correction Patch]** Finalized primary repository notation in Appendix A to match header metadata and Sec 8 as `github.com/deundeuni/soma-moa` account structure, completing document-wide citation consistency.
...
* **[L2] Auxiliary Guidance & Specialized UI Layer**
  * For General Evacuees / Crew / Ship Passengers: Green visible light, spatial audio, AR/MR glasses Visual HUD 3D guide lines, Co-Survival Bridge (earphones/wristbands/rings/etc.), smart rings, personal smartwatch/band, haptic vest/insole, spacesuit-embedded and ship lifejacket-linked left/right directional guidance, WebAR/voice evacuation assistance.
  * For Firefighters / Rescuers / EVA Crew / Maritime Rescuers: IR/NVG/Thermal high-contrast visual cues, rescuer-dedicated tactile/directional acoustic guidance, backend telemetry transmission.
  * For Unmanned Robotics / AMR / Drones: Visual SLAM/LiDAR 0-point position error reset fabric, survivor tracking telemetry.
  * For Control & Forensics: Post-disaster blackbox data recovery and building/ship/orbital module bottleneck report generation.
...
* **[L0] Infrastructure & Optical/Acoustic/RF Layer**
  * ISO 7010 exit signs, IMO SOLAS LLL (Low Location Lighting), indoor hydrant/fire alarm/space module/ship cabin internal/external location indicators (Red LED).
  * Hydrant box, ship fire-rated bulkhead (Bulkhead), and space module fire-rated internal chamber heat-resistant non-volatile flash memory (Blackbox module).
  * Directional speaker/acoustic anchors, underground parking/deep underground tunnel/ship corridor/spacecraft raceway multi-wavelength emitter, acoustic, and BLE Auracast/UWB modules.
...
* **Justification for Lightweight Edge Models (sLLM / Small Vision-Audio Model / Edge AI):**
  Disaster sites presuppose power outages, main server destruction, and total communication blackout. Furthermore, power and compute resources on AR glasses, Co-Survival Bridges, smart rings, smartwatches, ship lifejacket receivers, and spacesuit modules are severely constrained. Accordingly, on-device lightweight models (sLLM, Small Vision/Audio Models, NPU firmware AI engines) optimized for ultra-low power and ultra-low latency execution are presented as core implementation embodiments.
* **Core AI Estimation & Governance Roles:**
  * Multispectral Visual, AR Vision, and Spatial Acoustic/RF Perception: Asynchronously isolates and perceives ISO pictograms, SOLAS LLL lighting, hydrant indicators, raceway IR emitters, acoustic anchors, BLE Auracast/UWB, and small cell signals despite fire smoke, noise, glare, ship listing, or microgravity smoke dispersion.
  ...
  * Multi-Device Graceful Fallback Control: Autonomously transfers output control between AR HUD visual guidance and tactile haptic/spatial audio based on fluctuations in smoke/noise density or sight obstruction, actively suppressing hallucinated guidance upon confidence drops.
  * Data Feedback via Blackbox Logs: Feeds recovered blackbox logs back into AI training datasets to refine real-time rerouting algorithms by reflecting actual human/crew/passenger behavior patterns and facility bottlenecks.

---

## 3. Core System Blocks and Operational Mechanisms

### A. ANCHOR Signage, Hydrant/Terminal Boxes, Ship Bulkheads, Acoustic/RF Anchors, and Raceway Sensing (Infrastructure Domain)
* Considers ISO 7010 exit signs, IMO SOLAS LLL, hydrant/control panel location indicators, ship fire-rated bulkhead (Bulkhead) internal box modules, and ceiling/wall raceway optical/acoustic/BLE Auracast/UWB modules as absolute reference points (ANCHOR).
* Indoor hydrant boxes and ship fire-rated bulkhead chambers serve as fire-rated facilities under relevant fire safety regulations (see Section 0.2), operating as fixed anchors with guaranteed physical survival during fires and flooding unlike standard commercial markers or arbitrary beacons.
* Performs multisensory position 0-point calibration under ground fires with severe zero-visibility conditions, ship listing, and microgravity smoke blockades by receiving spatial audio, frequency-modulated signals, and BLE/UWB emergency broadcasts.
* Controls multi-channel reception of NVG/Thermal visual cues, tactile vibration signals, and specialized frequency data for firefighter, rescuer, maritime search-and-rescue, and astronaut equipment.
...
* **Raceway Route Alignment via Standard Lighting Fixtures & Ship LLL:** Raceway anchoring targets standard commercial/public/industrial/shipboard lighting fixtures (surface-mounted lights, IMO SOLAS LLL strips) rather than specialized decorative lights. While raceways and ship low-location lighting protect wiring and form cable paths, standard lighting fixtures are fixed along these routes per design conventions, aligning raceway paths closely with human evacuation routes.
* **Elevated Line-of-Sight & Crowd Occlusion Mitigation:** The elevated physical positioning of raceways along ceilings/walls and lower ship corridor walls offers optical and RF signal transmission advantages that mitigate line-of-sight blockages caused by dense evacuating crowds during disasters.
...
* Calibration & Estimation Algorithm:
  ...
  * Process: AI state estimator, Extended Kalman Filter (EKF), Visual SLAM drift reset, and acoustic triangulation/Doppler correction real-time calibrate the cumulative drift error matrix of inertial/gyroscope sensors to 0 points.
...
* Mandatory Emergency Power Linkage: Must link to building, ship emergency generators, and space module emergency power systems or include auxiliary independent power units (batteries, supercapacitors, energy harvesters).
...
The Co-Survival Bridge encompasses earphones, wristbands, smart rings, AR glasses, patches, haptic vests, smart shoes/insoles, ship lifejacket-embedded transducers, smart apparel, and spacesuit (IVA/EVA) inner linings. Conceived to reconnect isolated humans to safety networks, the device is officially named the "Co-Survival Bridge."

To suppress theft or misuse for personal entertainment, non-essential functions (such as music playback) are excluded. The device is deployed in public infrastructure and ship cabins as a single-purpose emergency auxiliary tool housing only necessary haptic reception components.

* **Standard Haptic Compass Protocol:**
  * Turn Left: Left temple/wristband/ring/watch/ship lifejacket/spacesuit left transducer double-tap (Left: 100ms ON / 100ms OFF / 100ms ON)
  * Turn Right: Right temple/wristband/ring/watch/ship lifejacket/spacesuit right transducer double-tap (Right: 100ms ON / 100ms OFF / 100ms ON)
...
* Emergency State Switch & 3-Second Onboarding: Upon fire/emergency detection, RF broadcasting switches to active ON. Scanning passive powerless QR/ArUco tags or tapping passive NFC tags attached to exit signs, hydrants, ship cabin walls, or space modules executes an app-less WebAR and OS deep-link (`last-light://{anchor_id}?b={broadcast_id}&k={public_temp_key}`), joining the channel within 3 seconds.
...
* Automatic Search During Sight Blockade: If smoke, ship flooding, or zero-gravity dispersion renders optical tag scanning impossible, terminals, bridges, AR glasses, smart rings, and linked smartwatches automatically search and join the strongest RF broadcast signals from two or more adjacent anchors.
* Tag Installation Principle: Non-powered passive tags containing no personal data are installed without obscuring statutory sign surfaces or IMO SOLAS indicators.

### G. Multisensory Anchor-Based Evacuation Structural Concept Specification
* Field Context: Presupposes extreme disaster environments such as underground parking garages, subterranean malls, deep underground tunnels, cruise/passenger ship cabin compartments, or space station modules where fire smoke and blackout severely restrict visibility.
...
* Multi-Anchor Composite Visualization: ISO 7010 green exit signs, IMO SOLAS LLL lights, and red hydrant indicators serve as absolute visual references on walls, while ceiling/wall raceway acoustic and RF modules project wireless broadcast signals through space in an organically unified layout.

### H. Distributed Heat-Resistant Local Blackbox Telemetry
* Physical Survival Assurance via Fire-Rated/Impact-Resistant Compartments: Physical and regulatory survival grounds for L0 blackbox memory receptacles in ground hydrant boxes (domestic NFPC, etc.), ship fire-rated bulkheads (IMO SOLAS, etc.), and space module chambers reference Sections 0.2 and 3.A, housed within protected fire-rated structures.
...
* Forensics & Liability Mitigation: Allows recovery of blackbox chips from surviving hydrant boxes, ship bulkheads, or module chambers following server destruction, structural collapse, or ship capsizing to objectively reconstruct evacuation paths and verify system performance.
...
* **Standard Compliance:** Cites ISO 7010, IMO SOLAS (International Convention for the Safety of Life at Sea), Bluetooth SIG, UWB, 3GPP Small Cell Specs, national fire codes, National Fire Performance Codes (NFPC), and infrared/visible light/acoustic standards as illustrative public references without claiming exclusive re-interpretation.
* **Non-Substitutability for Statutory Equipment:** Operates purely as an auxiliary reference system and does not replace the legal duties, performance, or installation standards of emergency exit lights, IMO LLL, alarm systems, hydrant indicators, or public address systems mandated by fire, building, ship safety, or space safety codes.
* **Safety Margin & Physical Limitations:** Auxiliary functions may delay or halt under extreme smoke, noise, ship capsizing/flooding, or RF shielding. Final evacuation decisions remain the responsibility of legal safety facilities and human rescue personnel.

---

## 6. Future Applications & Industrial Expansion Scope

* **Smart Underground Parking, Deep Tunnels, and Ship Raceways:** Deploys IR/visible dual-wavelength, directional acoustic, and BLE Auracast/UWB anchors along raceways for autonomous parking/exploration/logistics robot positioning during normal times and evacuation guidance during disasters.
* **Underground/Tunnel Telecom Infrastructure Evolution & Subordinate Backhaul Integration:** Interoperates with 5G/6G small cell and DAS networks in underground parking, tunnels, deep underground facilities, and subterranean malls as auxiliary position fusion and telemetry backhaul, maintaining offline zero-downtime autonomy under fixed infrastructure anchors even during total network failure.
* **Large Passenger Ship & Cruise Ship Cabins & Marine Muster Stations:** Utilizes cruise ship enclosed corridors, fire-rated bulkheads, and Muster Station signs as L0 0-point anchors, providing intuitive auxiliary directional guidance to muster stations via passenger lifejacket-embedded haptics or smart rings even during ship blackout or listing.
* **Hydrant & Orbital Module 3D Anchors:** Uses hydrant box surfaces, ship fire-rated bulkheads, and space station module internal chambers as 0-point calibration references to maintain tactile vibration guidance paths amidst severe visibility conditions.
...
* **Autonomous Logistics (AMR) & Rescue Robot Navigation:** Enables automatic Visual SLAM error calibration and survivor tracking in wireless-communication-impaired environments, underground facilities, ship interiors, or space modules using raceway and hydrant/bulkhead anchors.
...
* **Comprehensive Scope:** All concepts disclosed herein—including ANCHORs, ESTIMATION, Co-Survival Bridges (across all physical form factors like smart rings, vests, insoles, lifejacket receivers), AR Glasses Visual HUDs, IMO SOLAS LLL linkage, deep tunnel anchors, heterogeneous multi-device integration and Graceful Fallback, WebAR/NFC passive quick release, Visual SLAM Rapid Drift Reset, Auracast/UWB haptic compasses, underground small cell/DAS auxiliary integration, fire-rated hydrant and ship bulkhead physical anchoring, Tri-State Isolation, T-Reg Suppressor, distributed blackbox logs, spacesuit haptic guides, and AI self-evolution logic—apply broadly to secure comprehensive prior art coverage.
...
* Primary Repository: github.com/deundeuni/soma-moa
...
* **AI Assistance Disclosure —** All technical architectures, haptic logic, calculation formulas, and legal defense structures within this white paper belong strictly to the intellectual labor and field intuition of the sole human inventor (deundeuni). Artificial intelligence tools (AI models) were utilized strictly under the explicit direction of the inventor as auxiliary text editing, translation, and layout formatting tools (Auxiliary Text Editing Tools), and were defined under generalized tool scopes to mitigate potential brand-specific index or stake claims by specific corporations. Furthermore, artificial intelligence tools were not involved in the original technical ideation or invention process.
...
* **Non-Substitutability for Statutory Equipment:** Technologies disclosed in this specification (LAST-LIGHT v5.2), associated software, QR/ArUco/NFC tags, BLE Auracast/UWB fabrics, spatial acoustic anchors, underground telecom integration modules, IMO SOLAS LLL linkage modules, Co-Survival Bridges (in all form factors including smart rings, vests, insoles, lifejacket receivers), AR Glasses Visual HUDs, smartwatch linkage software, and distributed blackboxes do not replace the legal, physical, or functional duties of statutory emergency exit lights, IMO Low Location Lighting, alarms, or fire equipment under national fire, building, or ship safety codes, operating strictly as auxiliary reference tools.
* **Limitation of Liability:** The author (deundeuni / soma-moa) and open-source contributors assume no civil or criminal liability for evacuation delays, communication failures, sensory perception errors, data losses, or personal/property damages resulting from the implementation or application of this document, code, or visual materials. All safety management and evacuation responsibilities remain with site safety managers, ship captains/safety officers, evacuees, and statutory equipment operators.
* **Intellectual Property & Trademark Disclaimer:** Technical standards (Bluetooth SIG, ISO, IMO, 3GPP, etc.) cited herein serve solely as explanatory public examples without intent to infringe upon third-party trademarks. All technical expressions shall be interpreted as abstract, generic engineering concepts unconstrained by specific commercial brands.
